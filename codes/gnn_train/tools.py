...
class HetAgg(nn.Module):
	def __init__(self, args, feature_list, a_neigh_list_train, p_neigh_list_train, v_neigh_list_train,\
		...
		self.attentions = [GraphAttentionLayer(args, concat=True) for _ in
						   range(nheads)]
	...
	def a_content_agg(self, id_batch):
		...
		a_text_embed_batch_4 = self.feature_list[6][id_batch, embed_d * 3: embed_d * 4][0]
		a_text_embed_batch_5 = self.feature_list[6][id_batch, embed_d * 4: embed_d * 5][0]

		concate_embed = torch.cat((a_net_embed_batch, a_text_embed_batch_1, a_text_embed_batch_2,\
		 a_text_embed_batch_3, a_text_embed_batch_4, a_text_embed_batch_5), 1).view(len(id_batch[0]), 6, embed_d)
	...
	def p_content_agg(self, id_batch):
		...
		p_net_embed_batch = self.feature_list[4][id_batch]
	...
	def node_het_agg(self, id_batch, node_type):
		...
		x = torch.zeros(len(c_agg_batch), self.embed_d).cuda()
		for att in self.attentions:
			x.add_(att(node_type, c_agg_batch, concate_embed, a_agg_batch, p_agg_batch, v_agg_batch))
		x = x/8
		return x
		...
		if triple_index == 0:
			...
			for i in range(3):
				...
				if i == 0:
					...
					id_batch = self.v_train_id_list[batch_number * save_batch_s : ]
					out_temp = self.node_het_agg(id_batch, 3)
				out_temp = out_temp.data.cpu().numpy()
				for k in range(len(id_batch)):
					index = id_batch[k]
